...
function list_archive() {
	...
	case $format in
		...
		*[bgx]z*) tar tf "$current" | awk '{ if("'$selection'") s = (/^('"$(sed 's/[][\(\)\/]/\\&/g' <<< "$multiple_files")"')$/) ? " " : " "
			print s $0 }';;
		#*[bgx]z*) tar tf "$current" | awk '{ if("'$selection'") s = (/^('"${multiple_files//\//\\/}"')$/) ? " " : " "; print s $0 }';;
		zip) nr=2 flag=-l;;
		rar) nr=7 flag=l;;
		#zip) column=4 flag=-l;;
		#rar) column=5 flag=l;;
	...
	#	{ f = ""; for(c = '$column'; c < NF; c++) f = f $c " "
	#		if("'$selection'") s = (f ~ /^('"$(sed 's/[][\(\)\/]/\\&/g' <<< "$multiple_files")"')$/) ? " " : " "; print s f $c }'
	#		#if("'$selection'") s = (/^('"${multiple_files//\//\\/}"')$/) ? " " : " "; print s f $c }'
}
		...
		 )
			if [[ $options ]]; then
				...
				if [[ $multiple_files ]]; then
					...
					un_set selection list multiple_files
				else
			...
			case $option in
				...
				*)
					if [[ $options == bookmarks ]]; then
						...
						if [[ $archive && $(tar tf $archive | grep "$@") ]]; then
							set archive_single "$@"
							un_set list
						else
							set regex "'$current'/$@"
