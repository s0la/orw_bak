...
function apps() {
	...
	if [[ $(which apt 2> /dev/null) ]]; then
		...
		sudo pacman --noconfirm -R lxappearance-obconf-gtk3 lxappearance-gtk3 &> /dev/null
		...
		echo "Try installing them manualy, then run './setup.sh apps orw fonts man'"
		exit
	fi
